...
def within_same_week(date):
    if date is None:
        ...
        return (current_date.isocalendar()[1] == date.isocalendar()[1] and
...
def home():
    ...
    for wallet in wallets:
        ...
        for blockchain in blockchains:
            ...
            wallet_blockchain_transactions = [transaction for transaction in wallet.transactions if transaction.blockchain == blockchain]
            
            # Calculate the total volume
            volume = sum(transaction.volume for transaction in wallet_blockchain_transactions)

            # Get the date of the last transaction
            last_date = max(transaction.date for transaction in wallet_blockchain_transactions) if wallet_blockchain_transactions else None
    ...
    return render_template('home.html', wallets=wallets, blockchains=blockchains, 
                           transactions=transactions, matrix=matrix,
                           within_same_day=within_same_day, within_same_week=within_same_week, within_same_month=within_same_month)
...
def add_transaction():
    ...
    new_transaction = Transaction(volume=volume, wallet_id=wallet_id, blockchain_id=blockchain_id, date=date_obj)
...
@app.route('/wallet_blockchain_matrix')
def wallet_blockchain_matrix():
    wallets = Wallet.query.all()
    blockchains = Blockchain.query.all()
    ...
    for wallet in wallets:
        row = []
        for blockchain in blockchains:
            blockchain_transactions = [transaction for transaction in wallet.transactions if transaction.blockchain == blockchain]
            volume = sum(transaction.volume for transaction in blockchain_transactions)
            date = max(transaction.date for transaction in blockchain_transactions) if blockchain_transactions else None
            row.append((blockchain.name, volume, date))
        matrix.append((wallet.name, row))
...
def download_db():
    ...
    db_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'instance', 'airdrop.db')
